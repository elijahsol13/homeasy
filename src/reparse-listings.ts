...
import { createDatabase, closeDatabase } from './database/db';
...
import { CANONICAL_AMENITIES } from './config/amenities';
import { chromium } from 'playwright-extra';
import stealthPlugin from 'puppeteer-extra-plugin-stealth';
import { parseProxyConfig } from './modules/parser/proxy';
import { env } from './config/env';
import { extractPostsFromFbGraphQL } from './modules/parser/facebook.scraper';
...
async function reparseFacebook(db: any, limit?: number): Promise<void> {
  ...
  for (const row of targets) {
    ...
    try {
      ...
      const graphQLText: any = fullTextFromGraphQL;
      const bestText = (graphQLText && graphQLText.length > domText.length) ? (graphQLText as string) : domText;
